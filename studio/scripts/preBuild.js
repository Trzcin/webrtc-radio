...
const generateProdTSConfig = () => {
  ...
  const tsconfigElectronJSONPath = join(
    __dirname,
    '..',
  ...
  const newTsconfigSvelteJSONPath = tsconfigSvelteJSONPath.replace(
  ...
  writeFileSync(
    newTsconfigElectronJSONPath,
    JSON.stringify(tsconfigElectronJSON)
  );
};
